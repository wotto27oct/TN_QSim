...
for error in range(trial):
    ...
    # after applying error-correction pauli,
    # try to find more compact PEPDO (optimal truncation)
    print("------ optimal truncation ------")
    print("before")
    print(pepdo.horizontal_virtual_dims)
    print(pepdo.vertical_virtual_dims)
    print(pepdo.inner_dims)
    ...
    if width == 3:    
        trace_ori = np.trace(pepdo.calc_trace())
        rho_ori = pepdo.contract().reshape(2**qnum, -1) / trace_ori
        pepdo.nodes[0].tensor /= np.sqrt(trace_ori)
        ori_tensor = pepdo.tensors
        trun_vertical_list = []
        for h in range(height-1):
            for w in range(width):
                trun_vertical_list.append([h*width+w, 3])
        trun_horizontal_list = []
        for h in range(height):
            for w in range(width-1):
                trun_horizontal_list.append([h*width+w, 2])
        for nidx, eidx in trun_horizontal_list:
            pepdo.find_optimal_truncation(nidx, eidx, 4)
        for nidx, eidx in trun_vertical_list:
            pepdo.find_optimal_truncation(nidx, eidx, 4)
        ...
        print(pepdo.vertical_virtual_dims)
        print(pepdo.inner_dims)
        print("trace", pepdo.calc_trace())
        rho = pepdo.contract().reshape(2**qnum,-1)
        print("l2-norm", np.linalg.norm(rho - rho_ori))
        print("regularized l2-norm", np.linalg.norm(rho/np.trace(pepdo.calc_trace()) - rho_ori))

        pepdo.nodes[0].tensor /= np.sqrt(np.trace(pepdo.calc_trace()))
        pepo1 = PEPO([tensor.transpose(0,5,1,2,3,4) for tensor in pepdo.tensors], height, width)
        pepo2 = PEPO([tensor.transpose(0,5,1,2,3,4).conj() for tensor in ori_tensor], height, width)
        tree, cost, sp_cost = pepo1.find_pepo_trace_tree(pepo2)
        #print(f"slice: {tree.sliced_inds} total_flops: {tree.total_flops():,} cost: {cost:,} sp_cost: {sp_cost:,}")
        A = pepo1.calc_pepo_trace(pepo2, tree=tree)
        print("trace of inner-product between original and truncated:", np.trace(A))

    elif width == 5:
        trace_ori = np.trace(pepdo.calc_trace(algorithm))
        pepdo.nodes[0].tensor /= np.sqrt(trace_ori)
        ori_tensor = pepdo.tensors
        trun_vertical_list = []
        for h in range(height-1):
        ...
        for h in range(height):
            for w in range(width-1):
                trun_horizontal_list.append([h*width+w, 2])
        for nidx, eidx in trun_horizontal_list:
            pepdo.find_optimal_truncation(nidx, eidx, 4, 10, algorithm)
        for nidx, eidx in trun_vertical_list:
            pepdo.find_optimal_truncation(nidx, eidx, 4, 10, algorithm)
        pepdo.nodes[0].tensor /= np.sqrt(np.trace(pepdo.calc_trace(algorithm)))
